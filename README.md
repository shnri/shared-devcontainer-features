...
Featureを完全に同じreleaseへ固定する場合は`:2.2.0`、minor更新だけ受け取る場合は`:2.2`を使います。Dev Containerが生成する`devcontainer-lock.json`もconsumer repositoryへcommitし、実際に解決したversionとdigestを固定してください。
...
publish時に`2.2.0`、`2.2`、`2`、`latest`のOCI tagがFeature仕様に従って更新されます。Featureのソースとpackageに機密情報を含めないでください。
