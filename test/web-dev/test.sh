...
check "Claude shared marketplace commit" bash -c 'test "$(git -C "${CLAUDE_CODE_PLUGIN_SEED_DIR}/marketplaces/shared-agent-plugins" rev-parse HEAD)" = "f4f7091288f7e9d0d049e31ca9cd83cd226cb53c"'
...
check "Legacy Claude shared global instructions absent" bash -c '! test -e "${CLAUDE_CONFIG_DIR}/rules/shared-agent-plugins"'
check "Codex shared marketplace pinned" bash -c 'grep -Fq "ref = \"v0.23.0\"" "${CODEX_HOME}/config.toml"'
...
check "Legacy Codex shared global instructions absent" bash -c '! grep -Fqs "shared-agent-plugins:global-instructions" "${CODEX_HOME}/AGENTS.md"'
